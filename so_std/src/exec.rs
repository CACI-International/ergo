...
use so_runtime::{traits, traits::IntoTyped, types};
...
impl traits::GreaseStored for ExitStatus {
    fn put(&self, _ctx: &traits::StoredContext, mut into: ItemContent) -> so_runtime::Result<()> {
        bincode::serialize_into(&mut into, &self.0.clone().into_option()).map_err(|e| e.into())
    }

    fn get(_ctx: &traits::StoredContext, mut from: ItemContent) -> so_runtime::Result<Self> {
        let p: Option<i32> = bincode::deserialize_from(&mut from)?;
        Ok(ExitStatus(p.into()))
    }
}

so_runtime::grease_display_basic!(ExitStatus);
so_runtime::grease_type_name!(ExitStatus);
...
mod bytestream {
    ...
    impl From<so_runtime::types::String> for ByteStream {
        fn from(s: so_runtime::types::String) -> Self {
            Self::new(std::io::Cursor::new(s.into_bytes()))
        }
    }

    so_runtime::grease_type_name!(ByteStream);

    impl From<ByteStream> for so_runtime::types::String {
    ...
    impl traits::GreaseStored for ByteStream {
        ...
        fn get(_ctx: &traits::StoredContext, from: ItemContent) -> so_runtime::Result<Self> {
