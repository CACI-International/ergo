...
mod test {
    ergo_script::tests! {
        fn getopt(t) {
            t.assert_eq("self:getopt [a,b,c,d]", "fn a b c d");
            t.assert_eq("self:getopt [--flag,pos1,\"--key=a\",pos2,pos3,\"--key2=123=b\"]",
                "fn ~flag ~key=a ~key2=\"123=b\" pos1 pos2 pos3");
        }

        fn consecutive(t) {
            t.assert_eq("self:getopt ~consecutive [--a,--b,c,--d]", "fn ~a ~b c --d");
        }
    }
}
